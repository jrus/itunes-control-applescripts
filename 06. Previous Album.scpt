  ...
  tell application "iTunes"
    if player state is playing then
      with timeout of 10 seconds
        ...
        set testTrack to curTrack
        set loopEnder to 0
        repeat while loopEnder is 0
          try
            set testTrack to file track before testTrack
            set testAlbum to album of testTrack
            if testAlbum is not curAlbum then
              play (first file track of current playlist whose album is testAlbum)
              set loopEnder to 1
            end if
          on error
            stop
